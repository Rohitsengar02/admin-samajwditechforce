...
import { View, Text, ScrollView, TouchableOpacity, TextInput, Dimensions, Platform, ActivityIndicator, RefreshControl, Alert } from 'react-native';
import { LinearGradient } from 'expo-linear-gradient';
import { useRouter, useFocusEffect } from 'expo-router';
import {
    MapPin, Plus, Users, Building2, Edit, Trash2,
...
const DistrictCard = ({ district, onEdit, onDelete }: any) => (
...
export default function DistrictsPage() {
    ...
    const totalAssemblies = districts.reduce((sum, d) => sum + (d.assemblyCount || 0), 0);
    const totalMembers = districts.reduce((sum, d) => sum + (d.totalMembers || 0), 0);
            ...
            <View className="relative overflow-hidden mb-6">
                <LinearGradient colors={['#4F46E5', '#7C3AED']} className="pt-8 pb-12 px-6 rounded-b-[40px]">
                    ...
                    <View className="flex-row justify-between items-center mb-6">
                        <View className="flex-1">
                            <Text className="text-white text-3xl font-bold mb-2">Districts</Text>
                            <Text className="text-indigo-200 text-sm">Manage all districts</Text>
                        </View>
                        <TouchableOpacity
            ...
            <View className="px-4">
                ...
                <View className="mb-6">
                    <View className="flex-row items-center justify-between mb-4 px-2">
                        <Text className="text-lg font-bold text-gray-800">All Districts ({filteredDistricts.length})</Text>
                    </View>
                    ...
                    {loading && !refreshing ? (
                        ...
                        <View className="items-center py-10">
                            ...
                            <Text className="text-gray-400 font-medium text-lg mt-4">No districts found</Text>
                        ...
                        <View className="flex-row flex-wrap -mx-2">
                            {filteredDistricts.map((district: any) => (
                                <View key={district._id} className="w-full md:w-1/2 lg:w-1/3 p-2">
                                    <DistrictCard
                                        district={district}
                                        onEdit={(d: any) => router.push(`/(admin)/districts/${d._id}` as any)}
